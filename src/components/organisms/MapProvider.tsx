...
const createPillIcon = (price: number, priceType: string, isActive: boolean) => {
  ...
  const tail = isActive
    ? `<span style="
        ...
        width:10px;height:10px;background:#7C3AED;
        border-radius:1px;box-shadow:2px 2px 4px rgba(124,58,237,0.25);z-index:-1;
      "></span>`
    : "";
  ...
  return L.divIcon({
    html: `
      <div style="
        ...
        background:${bg};color:${color};padding:6px 10px;border-radius:100px;
        font-family:'Hanken Grotesk',sans-serif;font-size:13px;font-weight:600;
        box-shadow:${shadow};border:${isActive ? "none" : "1.5px solid #E5E7EB"};
...
function calcCardPosition(
  ...
  let top: number;
  if (placeAbove)       top = markerPx.y - pillHeight / 2 - CARD_HEIGHT - CARD_OFFSET;
  else if (placeBelow)  top = markerPx.y + pillHeight / 2 + CARD_OFFSET;
  else                  top = markerPx.y - CARD_HEIGHT / 2;
...
// ─── ✅ Map Controller (Untuk auto-pindah map & update letak popup) ───
function MapController({ 
  ...
  properties: Property[];
  onUpdatePx: (x: number, y: number) => void;
}) {
  const map = useMapEvents({
    move: () => updatePos(),
    zoom: () => updatePos(),
  ...
  const updatePos = useCallback(() => {
    if (activeId) {
      const p = properties.find(prop => prop.id === activeId);
      if (p) {
        const point = map.latLngToContainerPoint([p.location.lat, p.location.lng]);
        onUpdatePx(point.x, point.y);
      }
    }
  }, [activeId, properties, map, onUpdatePx]);

  useEffect(() => {
    if (activeId) {
      const p = properties.find(prop => prop.id === activeId);
      if (p) {
        // Map akan otomatis terbang mulus ke lokasi property yang di-klik
        map.flyTo([p.location.lat, p.location.lng], 15, { animate: true, duration: 1.5 });
        updatePos();
      }
    }
  }, [activeId, properties, map, updatePos]);
  ...
  return (
    <div
      style={{
        ...
        zIndex: 1000, width: `${CARD_WIDTH}px`, background: "#FFFFFF",
        borderRadius: "16px", overflow: "hidden",
        boxShadow: "0 8px 32px rgba(0,0,0,0.16)",
        fontFamily: "'Hanken Grotesk', sans-serif",
        animation: "popupIn .2s ease", pointerEvents: "auto",
      }}
    ...
    >
      ...
      <div style={{ position: "relative", height: "160px" }}>
        ...
        <button onClick={onClose} style={{
          ...
          boxShadow: "0 1px 4px rgba(0,0,0,0.2)", fontSize: "14px", color: "#374151", lineHeight: 1,
        }}>×</button>
      </div>
      <div style={{ padding: "14px 16px 16px" }}>
        ...
        <p style={{
          margin: "4px 0 2px", fontWeight: 700, fontSize: "15px", color: "#101828",
        ...
        <div style={{ display: "flex", gap: "16px", borderTop: "1px solid #F3F4F6", paddingTop: "10px" }}>
          {[
            { icon: "M2 9V19M22 9V19M2 14H22M2 9C2 7.89543 2.89543 7 4 7H20C21.1046 7 22 7.89543 22 9M6 7V5C6 4.44772 6.44772 4 7 4H17C17.5523 4 18 4.44772 18 5V7", label: `${property.bedrooms} Beds` },
            { icon: "M4 12H20M20 12V18C20 19.1046 19.1046 20 18 20H6C4.89543 20 4 19.1046 4 18V12M20 12V8C20 6.89543 19.1046 6 18 6H6C4.89543 6 5 7 5 8", label: `${property.bathrooms} Baths` },
            { icon: "M21 3L14 10M3 21L10 14M21 3H15M21 3V9M3 21H9M3 21V15", label: `${property.squareArea} m²` },
          ].map(({ icon, label }) => (
...
interface MapProviderProps {
  ...
  onMarkerActivate?: (id: string | null) => void;
}
...
export default function MapProvider({
  ...
  // ✅ FIX: Semua Hooks (useCallback, useEffect) WAJIB ditaruh di atas sebelum kondisi return (if) apa pun!
  const handleUpdatePx = useCallback((x: number, y: number) => {
    setMarkerPx({ x, y });
  }, []);
  ...
  useEffect(() => {
    if (externalActiveId !== null) {
      setInternalActiveId(externalActiveId); // Munculkan popup card!
    } else {
  ...
  // ✅ Early Return ditaruh SETELAH semua hooks selesai dideklarasikan
  if (!isClient) {
  ...
  const highlightedId = internalActiveId;

  const popupProperty = internalActiveId
    ? properties.find((p) => p.id === internalActiveId) ?? null
    : null;
  ...
  const handleActivate = (id: string, px: number, py: number) => {
    ...
    onMarkerActivate?.(id); // sync ke parent
  };

  const handleDeactivate = () => {
    setInternalActiveId(null);
    setMarkerPx(null);
    onMarkerActivate?.(null);
  };

  return (
    <div ref={containerRef} className="h-full w-full relative" style={{ isolation: "isolate" }}>
      {popupProperty && markerPx && (
        <PropertyPopupCard
